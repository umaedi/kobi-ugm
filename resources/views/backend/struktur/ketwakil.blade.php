...
<script src="{{ asset('backend') }}/js/sweet_alert.min.js"></script>
    <script>
        $('#formKetuaWakil').submit(function(event){
        ...
        $.ajax({
            ...
            if(response.status == 201) {
                swal({
                  ...
                  .then(() => {
                    window.location.replace(BaseUrl+'/admin/organisasi/ketua-wakil');
                  });
            }else {
    ...
    $('#formUpdateStructure').submit(function() {
        ...
        $.ajax({
            url: BaseUrl+'/api/admin/leader/'+id,
            data: data,
            method: 'POST',
            processData: false,
            contentType: false,
