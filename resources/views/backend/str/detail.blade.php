...
<div class="container-fluid">
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h3 class="h5 mb-0 text-gray-800 d-inline mr-5">Detail Pengajuan STR Yang Di Tolak</h3>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
              ...
              <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <div class="form-group mb-4">
                        <label>{{ __('Alsan Penolakan') }}</label>
                        <textarea type="text" class="form-control my-colorpicker1">{{ $str->pesan }}</textarea>
                      </div>
                      <hr>
...
<script>
  $('#verifStr').submit(function(event) {
      ...
      complete: (response) => {
          if(response.status == 201) {
            sendMail(response.responseJSON.data);
    ...
    function sendMail(data) {
      $.ajax({
          url: BaseUrl+'/api/admin/sendmail/str/verif',
          headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
  ...
  $('#rejectStr').submit(function(event) {
    ...
    function sendMail(data) {
      $.ajax({
          url: BaseUrl+'/api/admin/sendmail/str/reject',
          headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
          data: {email: data.email, name: data.name, message: data.message},
          method: 'POST',
          complete: (res) => {
            if(res.status == 200) {
              console.log('Email terkirim ke '+data.email);
