...
<div class="container-fluid">
  ...
  @component('components.backend.card-table')
    @slot('header')
      <h3 class="h5 mb-0 text-gray-800 d-inline mr-5">Draft Berita Atau Artikel</h3>
    @endslot
    @slot('dropdown')
...
<script>
  let table = $("#dataTable").DataTable({
    processing: true,
    serverSide: true,
    ajax: BaseUrl+'/api/admin/posts/draft/0',
    columns: [
      ...
      {data: 'publish_at', name: 'publish_at'},
      {
        ...
        return `
        <a href="/post/show/` + row.slug +`" type="button" target='_blank' class="btn btn-sm btn-success"><i class="fas fa-eye text-white"></i></a>
        <a href="post/` + row.slug +`/edit" type="button" class="btn btn-sm btn-warning"><i class="fas fa-eye text-white"></i></a>
